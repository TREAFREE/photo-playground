...
4. 独立发布可以使用 `<name>-vX.Y.Z` 标签，只上传该 ZIP。若使用这种发布方式，应把 README 下载入口指向各自固定 release，不能再依赖整个仓库的 latest 链接。合集 Release 可同时附上多款独立 ZIP；每款 VERSION 与根目录 skills-catalog.json 单独记录规则版本。历史 Release 附件不覆盖。
