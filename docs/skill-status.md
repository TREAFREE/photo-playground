...
发布日期版本与本地安装是两件事。v0.4.0 已提供八款独立 ZIP；v0.5.0 提供十一款。个人技能列表只显示已安装且被当前环境发现的项目，不显示整个 GitHub 仓库。

本项目的状态使用 **已发布 / 样例已测**，不把少量漂亮样例称作普遍稳定性证明。当前只有 Codex 内置 image_gen 实测；豆包、其他模型、人脸身份保真、跨模型文字质量尚未验证。所有 Skill 使用独立规则和版本，可单独更新。
