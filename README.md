...
**豆包工作／工作任务有对话安装 GitHub Skill 的公开实操；我们的本机测试使用的是普通聊天，不能代表工作模式。**

有技能安装入口时，可新建技能并要求安装本仓库 `skills/具体名称`，保留完整引用资源。本仓库在豆包工作里的安装尚待实测，详见 [步骤与证据](docs/doubao.md)。

普通聊天已实测可通过“照片＋Skill 正文”生成建筑版画：
